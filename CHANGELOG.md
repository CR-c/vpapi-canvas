...
+ [调整] 一次导入多张图片后会全部选中并出现共用提示词；窄屏时左侧列表先收起，提示词贴在屏幕底部。
+ [调整] 画布不再拉取、缓存或展示模型单价和生成消耗提示；顶栏剩余额度保持不变。
+ [修复] 图片任务轮询兼容原生图片二进制与 JSON 结果，视频按任务终态取回结果并保留已公布的音轨 / 水印参数。
+ [优化] Key 接入复用一次模型查询更新能力缓存，区分限流与余额不足，并说明自动分组和单 Key 全能力接入。
...
+ [修复] Canvas Agent 部署为本机 systemd 服务并通过 nginx 反代 /canvas-agent/，公网 HTTPS 页面可直接连接 Codex。
+ [修复] 视频参考图改为 JSON images 传递，配合网关自动将 base64 参考图托管为临时 HTTPS URL，kyy/TM/meai 渠道图生视频可用。
+ [调整] 视频请求按模型自动适配分辨率与时长挡位（如 MiniMax-H3-768p 固定 768p/10-15 秒），分辨率选项扩展 1080p/2K/4K。
+ [修复] 修复 zkki 网关 kyy 渠道视频任务查询（not_implemented）与 data URL 参考图托管转换。
+ [修复] dola-seedream 图生图不再发送不支持的 response_format 参数，参考图编辑恢复正常。
+ [修复] 视频参考图字段改为标准 input_reference，避免网关忽略参考图。
+ [修复] zkki API 网关 CORS 白名单加入 ic.735678.xyz，画布页可直接跨域调用图片/视频模型。
+ [调整] 不再内置固定的 zkki / zzone 模型与密钥，默认渠道统一为 vpapi，模型通过 `/v1/models` 按能力标签导入。
...
+ [修复] 视频轮询遇到单次网络抖动、网关 5xx 或限流时先重试再继续等待，网关仍在渲染的任务不再被误报「请求失败 / 生成失败」。
+ [修复] 视频结果地址兼容网关的 `metadata.url` 分享链接（vpapi 契约），跨域或不可达时自动回退到带鉴权的 `/videos/{id}/content`，已生成的视频不再因取回失败被判失败。
+ [调整] 视频创作台与画布节点统一 30 分钟轮询预算（创作台原为 5 分钟），seedance 等长耗时模型不再提前超时。
+ [新增] 改造为 vpapi 专用画布：网关与协议锁定为 vpapi，新用户首次打开自动进入接入引导，粘贴一个 API Key 即自动导入模型、设好默认项并进入创作。
+ [新增] 右侧内置助手：用 vpapi 的文本模型在浏览器里对话创作，可直接读取 / 修改画布节点、触发生图生视频、检索提示词与素材，写操作可设为需确认；上游本地 CLI Agent 保留为可选。
+ [新增] 顶栏显示当前 API Key 的剩余额度、已用额度与到期时间，并提供充值、创建 Key、查看文档的入口。
+ [调整] 设置页渠道管理替换为 vpapi 接入面板：协议切换、模型脚本、多渠道等入口不再暴露（上游实现保留在代码中，便于继续同步上游）。
+ [调整] 品牌与链接改为「vpapi 画布」：站点标题、图标、文档入口、GitHub 与版本检查地址均已切换。
+ [新增] 上游同步机制：`FORK.md` 补丁点清单 + `scripts/sync-upstream.sh` + 定时 `upstream-sync` Action，随时可合并 basketikun/infinite-canvas 的最新改动。
+ [新增] 内置助手的模型列表改为按网关端点能力挑选（含被判为图片/视频但同样支持 chat 的模型），支持工具调用的模型排在前面，Key 里没有纯文本模型时也能用上助手。
+ [修复] 内置助手遇到模型未开启 function calling 或上游对 tools 报错时，先重试再降级为纯对话并给出提示，不再直接报错中断；助手消息改为不可变更新，流式回复能正常回显。
+ [新增] 助手新增画布生成工具：说一句「生成图片 / 生成视频」即自动新建生成配置节点、连接参考节点并开始生成，写操作默认需要确认。
+ [新增] 接入改为按能力填 Key：文本 / 助手、生图、视频、音频可以分别使用不同分组的 Key，模型按各自的 Key 导入，请求自动走对应 Key；只填一把也能覆盖所有能力。
+ [调整] 额度优先显示账号总额度（读取 vpapi 的 `/api/usage/user`，sk- 可直接读）；网关未提供时回退到当前 Key 额度并标注口径与切换方式。
+ [调整] 内置助手只使用文生模型（默认取「文本 / 助手」那把 Key 的默认模型），生图 / 视频由它调用画布工具完成；没有文生模型时给出明确提示而不是列出图片模型。
+ [调整] 助手触发画布生成前会校验对应能力是否配了模型，缺失时直接提示补 Key，不再建出无效的生成节点。
+ [修复] 模型能力标签按调用方可见分组返回（vpapi 侧），图片模型识别关键字改为通用前缀，避免同一模型在不同分组间串类。
+ [修复] 同一把 Key 被浏览器自动填充到多个能力槽位导致模型重复导入：Key 输入框禁用密码自动填充，接入时拦截重复 Key，面板里提供一键清理并新增「更换 Key」。
+ [新增] 接入时自动拉取网关价格目录，生图 / 图生图 / 生成视频前提示本次预计消耗的额度（按次 × 张数、按秒 × 时长；拿不到价格时静默）。
+ [修复] 图生图选「auto」画幅时改为按第一张参考图的比例出图，不再由上游随机决定；gpt-image 系列自动吸附到 1024x1024 / 1536x1024 / 1024x1536。
+ [新增] 模型选择器（生图 / 视频 / 画布节点 / 助手）直接显示网关单价，多档位模型标「起」，拿不到价格时不显示。
+ [调整] 生成消耗提示按用户选的分辨率 / 质量 / 是否带参考图匹配计费档位（如 720p 带参考图与不带参考图分别计价），不再固定用第一档。
+ [调整] 视频轮询改为「软超时后继续等」：前 30 分钟每 2.5 秒查询，之后每 30 秒继续查到 2 小时，网关仍在渲染的任务不再被判「生成超时」。
+ [新增] 画布视频节点支持刷新后恢复：任务 id 本地持久化，刷新页面后自动继续查询并把结果回填原节点，不再显示「生成中断」。
+ [修复] 视频分辨率按网关挡位改为「手填值原样发送」：网关公布的挡位之外手填的分辨率不再被静默改回第一档（例如 seedance-2.0-mini-XG 公布 720p 时仍可用 480p），网关不支持时直接显示网关的报错；配置默认档与 auto 仍按网关挡位吸附。
+ [修复] 视频分辨率输入框显示当前挡位的数值（如「720p」显示为 720），不再因数值框无法解析挡位标签而显示为空。
+ [修复] 默认模型按能力校验：视频模型不会再被写进「生图」槽位（助手按名字指定模型时最容易触发），错配时清空该槽位让用户重选。
+ [修复] 助手的创作台工具（生图 / 生成视频）只接受对应能力的模型，传错能力时直接说明而不是写进槽位。
+ [修复] 内置助手请求被网络 / 代理 / 网关拦截时不再只显示浏览器的「Failed to fetch」，改为给出实际请求地址与排查提示；网关返回 JSON 错误时提取其中的说明文字，不再整段回显 JSON。
+ [调整] 接入改为两类 Key：文生（助手 / 文本）与媒体（图片 / 视频 / 音频），每组可接多把 Key 并按列表顺序表示优先级（可上移 / 下移），同一把 Key 不允许重复接入。
+ [调整] 模型选择器（画布节点、生成配置、创作台、设置默认模型与助手）按 Key 分组展示，条目改为「模型名 + 单价」两行排版并加宽弹层，长模型名不再被裁切遮挡。
+ [修复] 视频画幅按所选方向真实下发：之前只发 `size`（网关按分辨率处理、缺画幅时回落 16:9），选竖屏也会出横屏；现在同时下发 `ratio` / `aspect_ratio` / `metadata.ratio`，并按网关公布的 `aspect_ratios` 吸附（宽屏 1792x1024 → 16:9、长图 1024x1792 → 9:16），视频面板选「方形」也能得到 1:1；没选过尺寸时仍按横屏（生图默认档 1:1 不再被当成方形视频）。
