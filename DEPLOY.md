...
1. 粘贴你的 vpapi API Key（在 <https://vp.zkki.net/keys> 创建；注意别用渠道系统密钥）。
2. 点「接入」——画布会拉取该 Key 下的全部模型，按网关发布的能力标签识别生图 / 视频 / 文本 / 音频并设好默认模型。
3. 之后顶栏会常驻显示这把 Key 的剩余额度，点开可看已用额度、到期时间和充值入口。
