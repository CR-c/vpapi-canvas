...
const overrides: Record<string, Record<string, string>> = {
    "zh-CN": {
        "product.batch.title": "已选 {{count}} 张图，共用一条提示词",
        "product.batch.placeholder": "写一句提示词，每张图都会按它各自生成一张",
        "product.batch.generate": "批量生成",
        "product.batch.stop": "停止",
        "product.batch.partial": "{{count}} 张生成失败",
        ...
        "product.connect.hint": "到 vpapi 的「密钥」页面创建 Key，分组保持自动选择即可；画布会导入这把 Key 实际可用的模型。",
        ...
        "product.connect.empty": "这把 Key 下没有可用模型，请检查密钥的模型限制；若手动指定了分组，可改为自动选择后重新读取模型",
        "product.connect.invalidKey": "API Key 无效或已被禁用，请到 vpapi 重新复制",
        "product.connect.groupRequired": "这把 Key 的分组配置不可用，请到 vpapi 密钥页面改为自动选择后重试",
        "product.connect.quotaExhausted": "这把 Key 的额度不足，充值后再试",
        "product.connect.rateLimited": "请求过于频繁，请稍后重新读取模型；无需更换 Key 或充值",
        ...
        "product.connect.slotHint": "一把 Key 只需填一行，全部可用能力会自动导入。文生与媒体只是画布内的整理分类，不是 vpapi 的计费分组。",
    ...
    "en-US": {
        "product.batch.title": "{{count}} images selected, one shared prompt",
        "product.batch.placeholder": "One prompt. Each image generates its own result.",
        "product.batch.generate": "Generate all",
        "product.batch.stop": "Stop",
        "product.batch.partial": "{{count}} images failed",
        ...
        "product.connect.hint": "Create a key on the vpapi keys page with automatic group selection. The canvas imports the models actually available to that key.",
        ...
        "product.connect.empty": "This key exposes no models. Check its model restrictions, or switch a manually assigned group to automatic selection and reload models.",
        "product.connect.invalidKey": "The API key is invalid or disabled; copy a new one from vpapi",
        "product.connect.groupRequired": "This key's group is unavailable. Switch to automatic group selection on the vpapi keys page and retry.",
        "product.connect.quotaExhausted": "This key is out of quota; top up and retry",
        "product.connect.rateLimited": "Too many requests. Reload models shortly; you do not need a new key or a top-up.",
        ...
        "product.connect.slotHint": "Enter each key once to import all its available capabilities. Text and media organize keys in the canvas; they are not vpapi billing groups.",
